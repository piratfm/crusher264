...
static int  fx2hex2usb_16b(usb_dev_handle   *udev, FILE* fx2Fw);

/************************************************************
 * Uploading FX2 FIRMWARE
 ************************************************************/
int crusher_fx2loader(void	*handler, FILE* fx2Fw, int cpucs_send)
{
	...
	if(cpucs_send) {
        ret = usb_control_msg(udev, USB_TYPE_VENDOR + USB_RECIP_DEVICE,
                    A2_REQ, A2_CPUCS, 0x00, a2_reset_off, 0x01, USB_TIMEOUT);
        if (ret < 0){
    ...
    if(cpucs_send) {
        if (!fx2hex2usb_16b(udev, fx2Fw)){
            ERROR("FX2 firmware sending failed!");
            return 0;
        }

        ret = usb_control_msg(udev, USB_TYPE_VENDOR + USB_RECIP_DEVICE,
                    A2_REQ, A2_CPUCS, 0x00, a2_reset_on, 0x01, USB_TIMEOUT);
        if (ret < 0){
            ERROR("sending FX2 Reset failed, code: %d", ret);
            return 0;
        }
...
static int  fx2hex2usb_16b(usb_dev_handle   *udev, FILE* fx2Fw)
{
    ...
    for (;;) {
         ...
         * or when we've buffered as much as we can. */
         if (len != 0) {
            if (usb_control_msg(udev, USB_TYPE_VENDOR + USB_RECIP_DEVICE, A2_REQ,
                    off, 0x00, (char *) data, len, USB_TIMEOUT) < 0)
            {
                ERROR("Can't send %d bytes to 0x%04x address into device",
                        len, off);
                return 0;
